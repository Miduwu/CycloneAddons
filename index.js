...
bot.on("interactionCreate", async i => {
    ...
    if(i.customId.split("_")[0] == "ACCEPT") {
        ...
        await i.update({embeds: [new EmbedBuilder().setDescription("<:cyaddons_check:1060662306507333753> | ***`El bot fue aceptado.`***").setColor(Auxiliar.Colors.cyan)], ephemeral: true})
...
bot.on("interactionCreate", async i => {
    ...
    if(i.customId.split("_")[0] == "DECLINE") {
        ...
        .setFooter({text: i.user?.username, iconURL: i.user?.displayAvatarURL({size: 4096})})
        await i.update({embeds: [new EmbedBuilder().setDescription("<:cyaddons_error:1060665620468863096> | ***`El bot fue declinado.`***").setColor(Auxiliar.Colors.red)], ephemeral: true})
        memberbot.kick()
